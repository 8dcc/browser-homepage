...
function checkSearchEngine() {
    ...
    if (settings_global_object["use_se_icons"]) {
        ...
        se_icon.innerHTML = `<img src="img/search-engines/${user_search_engine.icon}" alt="Icon" height="${user_search_engine.icon_h}">`;
    } else {
        // Use block for text
        se_text.style.display = "block";
        se_text.innerHTML = `<div class="search-engine-text">${user_search_engine.text}</div>`;
    }
}
...
document.getElementById("search-form").addEventListener("submit", function(e) {
    ...
    if (search_query.value.trim() !== "") {
        if (user_search_engine) {
            ...
            var url = search_engines[default_search_engine].url + search_query.value;
        }
